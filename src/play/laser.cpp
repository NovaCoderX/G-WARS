...
void Laser::draw() {
	...
	displayVertices[BEGIN].x = horizontalProject(renderVertices[BEGIN].x,
			renderVertices[BEGIN].z);
	displayVertices[BEGIN].y = verticalProject(renderVertices[BEGIN].y,
			renderVertices[BEGIN].z);

	renderVertices[END] = staticVertices[END] * objectToCameraMatrix;

	displayVertices[END].x = horizontalProject(renderVertices[END].x,
			renderVertices[END].z);
	displayVertices[END].y = verticalProject(renderVertices[END].y,
			renderVertices[END].z);

	glColor3fv(laserColor[BEGIN].data);
	glVertex2i(displayVertices[BEGIN].x, displayVertices[BEGIN].y);

	glColor3fv(laserColor[END].data);
